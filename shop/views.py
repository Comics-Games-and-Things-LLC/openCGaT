...
def product_list(request, partner_slug=None):
    ...
    if form.is_valid():
        ...
        items = item_list_filter(
            ...
            has_barcode=form.cleaned_data.get('has_barcode'),
    ...
    if form.is_valid():
        ...
        if action == BulkEditItemsForm.UPDATE_PRICES:
            for item in items:
                ...
                item.save()
        if action == BulkEditItemsForm.UPDATE_DEFAULT_PRICES:
            for item in items:
                item.default_price = get_new_price(item, base_on_msrp, multiplier, price_override)
                item.save()
        if action == BulkEditItemsForm.UPDATE_MSRP:
            for item in items:
                item.product.msrp = price_override
                item.product.save()
                item.save()
